...
#
# Uso:
#   ./skinner.sh [--dry-run] [--max-loops N] [--prompt "task description"]
###############################################################################
...
PROJECT_ROOT="${PROJECT_ROOT:-workspace/motor-financeiro}"
...
WORKTREE_DIR=""
WORKTREE_BRANCH=""
LOG_FILE="$REPO_ROOT/.skinner/logs/session-$(date +%Y%m%d-%H%M%S).log"
NO_PROGRESS_COUNT=0
LAST_ERROR=""
SAME_ERROR_COUNT=0
LOOP_COUNT=0
...
create_worktree() {
    ...
    WORKTREE_BRANCH="ralph/work-$timestamp"
    WORKTREE_DIR="$REPO_ROOT/.claude/worktrees/ralph-$timestamp"
    ...
    # Copy .ralph and .ralphrc into worktree (Ralph needs them)
    cp -r "$REPO_ROOT/.ralph" "$WORKTREE_DIR/.ralph"
    cp "$REPO_ROOT/.ralphrc" "$WORKTREE_DIR/.ralphrc"
...
cleanup_worktree() {
    if [ -n "$WORKTREE_DIR" ] && [ -d "$WORKTREE_DIR" ]; then
        ...
        if [ -n "$changes" ]; then
            ...
            log "INFO" "To merge: git checkout 001-motor-financeiro && git merge $WORKTREE_BRANCH"
...
skinner_commit() {
    ...
    # Stage all changes in PROJECT_ROOT
...
Loop: $LOOP_COUNT | Tasks-this-session: $TOTAL_TASKS_COMPLETED
EOF
    )"
...
run_ralph_loop() {
    ...
    local output
    output=$($CLAUDE_CODE_CMD --print "$prompt" 2>&1) || true
...
main() {
    ...
    log "INFO" "Max loops: $MAX_LOOPS | Dry run: $DRY_RUN"
    log_separator
    ...
    if [ "$TOTAL_TASKS_COMPLETED" -gt 0 ]; then
        ...
        log "INFO" "  2. Diff vs base:    git diff 001-motor-financeiro..$WORKTREE_BRANCH"
        log "INFO" "  3. Merge if good:   git checkout 001-motor-financeiro && git merge $WORKTREE_BRANCH"
